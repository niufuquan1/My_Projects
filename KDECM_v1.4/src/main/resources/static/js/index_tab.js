...
(function($) {
	...
	if (urlHis.indexOf("literatureId") != -1) {
		var url = decodeURI(urlHis);
		var str = url.split("?")[1], items = str.split("&");
		var arr = items[0].split("=");
		//alert(url);
		$("#main")[0].src = "../../kdecm/literature/literatureDetail2.html?literatureId="
				+ arr[1] + "&userId=1&chapter=0";
	}
	$.dptab = {
		...
		addTab : function() {
			...
			if (flag) {
				...
				var str1 = '<iframe class="DP_iframe" id="iframe' + dataId
						+ '" name="iframe' + dataId
		...
		scrollTabRight : function() {
			var marginLeftVal = Math.abs(parseInt($('.page-tabs-content').css(
					'margin-left')));
			var tabOuterWidth = $.dptab.calSumWidth($(".content-tabs")
		...
		scrollToTab : function(element) {
			var marginLeftVal = $.dptab.calSumWidth($(element).prevAll()), marginRightVal = $.dptab
		...
		loadMenu : function(data) {
			...
			$.each(data, function(i) {
				...
				if (row.type == 1) {
					...
					_html += "<a href='" + row.url + "' data-id='" + row.menuId
							+ "' class='menuItem'><i class='" + row.icon
							+ "'></i> " + row.name + "</a>";
				}
				_html += "</li>";
			});
		...
		updatePassword : function() {
			dialogContent({
				...
				yes : function(index) {
					...
					var data = "pswd=" + vm.pswd + "&newPswd=" + vm.newPswd;
					$.ajax({
						type : "POST",
